...
-- Corrigir as políticas RLS para permitir que as funções do sistema criem jogos
...
-- Criar política mais permissiva para inserção de jogos pelo sistema
CREATE POLICY "Allow game creation by functions" 
ON public.games 
FOR INSERT 
WITH CHECK (true);

-- Permitir que o sistema atualize jogos
CREATE POLICY "Allow game updates by functions" 
ON public.games 
FOR UPDATE 
USING (true);

-- Usuários podem ver jogos onde participam
CREATE POLICY "Users can view games where they participate" 
ON public.games 
FOR SELECT 
USING (
  id IN (
...
-- Garantir que as funções executem com privilégios adequados
...
-- Recriar a função create_game_when_ready com SECURITY DEFINER
...
-- Também garantir que o trigger function tenha privilégios adequados
