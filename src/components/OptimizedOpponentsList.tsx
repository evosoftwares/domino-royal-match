...
// Componente individual memoizado com comparação otimizada
const OpponentCard = memo<{ player: ProcessedPlayer }>(({ player }) => {
  return (
    <div
      className={cn(
...
const OptimizedOpponentsList = memo<OpponentsListProps>(({ opponents }) => {
  return (
  ...
  // Comparação de array otimizada
  if (prevProps.opponents.length !== nextProps.opponents.length) return false;
  
  return prevProps.opponents.every((prev, index) => {
    const next = nextProps.opponents[index];
    return (
      prev.id === next.id &&
      prev.pieces.length === next.pieces.length &&
      prev.isCurrentPlayer === next.isCurrentPlayer &&
...
OptimizedOpponentsList.displayName = 'OptimizedOpponentsList';

export default OptimizedOpponentsList;
