-- Script para corrigir problemas no trigger da primeira peça
-- Execute no painel do Supabase (SQL Editor)

-- Primeiro, vamos corrigir a função play_highest_piece com uma lógica melhor para o próximo jogador
...
BEGIN
    RAISE NOTICE 'Iniciando play_highest_piece para game_id: %', game_id;
    
    -- Loop através de cada jogador para encontrar a peça mais alta
    FOR player_record IN 
        SELECT user_id, hand, position FROM game_players WHERE game_id = play_highest_piece.game_id ORDER BY position
    LOOP
    ...
    IF highest_piece_value > -1 AND piece_found IS NOT NULL THEN
        ...
        SELECT COUNT(*) INTO total_players FROM game_players WHERE game_id = play_highest_piece.game_id;
...
-- Atualizar a função do trigger para ter mais logs
...
-- Recria o trigger para garantir que está ativo
...
-- Para testar manualmente em um jogo existente (descomente a linha abaixo):
-- SELECT play_highest_piece('SEU-GAME-ID-AQUI'); 